...
class TestConfig < Minitest::Test
  ...
  def provider
    NemoGuardrails::Provider.new(name: 'test', base_url: 'http://endpoint.invalid/v1',
  ...
  def test_a_stock_folder_builds_an_engine
    Dir.mktmpdir do |dir|
      config = NemoGuardrails::Config.load(write_config(dir, config_yaml: STOCK))
  ...
  def test_the_engine_runs_the_flow_against_the_configured_model
    Dir.mktmpdir do |dir|
      ...
      chat = NemoGuardrails::Chat.new(model: 'some/instruct', http: http)
  ...
  def test_prompts_yml_becomes_the_policy_the_judge_sees
    ...
    Dir.mktmpdir do |dir|
      ...
      config = NemoGuardrails::Config.load(folder)
  ...
  def test_a_shipped_flow_file_is_loaded_and_runnable
    ...
    Dir.mktmpdir do |dir|
      config = NemoGuardrails::Config.load(write_config(dir, config_yaml: yaml, flows: { 'input' => flow }))
  ...
  def test_a_flow_nothing_defines_raises_at_load
    ...
    Dir.mktmpdir do |dir|
      config = NemoGuardrails::Config.load(write_config(dir, config_yaml: yaml))
      error = assert_raises(NemoGuardrails::ConfigError) { config.engine(provider: provider) }
      assert_includes error.message, 'invented flow'
    end
  end

  def test_a_missing_folder_raises
    assert_raises(NemoGuardrails::ConfigError) { NemoGuardrails::Config.load('/nowhere/at/all') }
  end
  ...
  def test_a_model_entry_endpoint_beats_the_ambient_provider
    Dir.mktmpdir do |dir|
      config = NemoGuardrails::Config.load(write_config(dir, config_yaml: STOCK))
      elsewhere = NemoGuardrails::Provider.new(name: 'elsewhere', base_url: 'http://other.invalid/v1',
  ...
  def test_a_provider_configuration_names_its_models_and_flows
    config = NemoGuardrails::Config.for_provider(provider)
  ...
  def test_a_written_folder_reads_back
    Dir.mktmpdir do |dir|
      out = NemoGuardrails::Config.for_provider(provider).write!(dir)
      reloaded = NemoGuardrails::Config.load(out)
  ...
  def test_a_round_trip_still_builds_an_engine
    Dir.mktmpdir do |dir|
      out = NemoGuardrails::Config.for_provider(provider).write!(dir)
      engine = NemoGuardrails::Config.load(out).engine(provider: provider)
      refute engine.empty?
    end
  end
end
