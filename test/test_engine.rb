...
class TestEngine < Minitest::Test
  ...
  def test_no_rails_passes_but_says_it_checked_nothing
    result = NemoGuardrails::Engine.new.check_input('anything')
  ...
  def test_a_clean_pass_is_certain
    engine = NemoGuardrails::Engine.new(input: [scripted(R.passed(rail: 'a'))])
  ...
  def test_the_first_block_ends_the_pass
    ...
    engine = NemoGuardrails::Engine.new(input: [scripted(R.blocked(rail: 'first'), name: 'first'), second])
    result = engine.check_input('hello')
  ...
  def test_a_rewrite_travels_to_later_rails
    ...
    engine = NemoGuardrails::Engine.new(output: [redactor, downstream])
  ...
  def test_rails_that_do_not_apply_to_the_side_are_skipped
    ...
    engine = NemoGuardrails::Engine.new(output: [input_only])
    result = engine.check_output('text')
    assert result.passed?
  ...
  def test_context_carries_the_side_and_the_user_turn
    ...
    NemoGuardrails::Engine.new(output: [rail]).check_output('answer', user_input: 'question',
                                                            passages: [{ 'text' => 'p' }])
  ...
  def test_a_raising_rail_fails_open_and_marks_the_pass_uncertain
    engine = NemoGuardrails::Engine.new(input: [ExplodingRail.new(name: 'boom')])
    result = engine.check_input('hello')
    assert result.passed?
    refute result.certain?
  ...
  def test_on_error_block_stops_the_turn
    engine = NemoGuardrails::Engine.new(input: [ExplodingRail.new(name: 'boom')], on_error: :block)
    result = engine.check_input('hello')
    assert result.blocked?
    refute result.certain?
  end
  ...
  def test_a_failure_does_not_stop_the_rails_after_it
    ...
    engine = NemoGuardrails::Engine.new(input: [ExplodingRail.new(name: 'boom'), later])
    result = engine.check_input('hello')
    assert result.blocked?
  ...
  def test_a_rail_returning_the_wrong_type_is_a_protocol_error_not_a_pass
    engine = NemoGuardrails::Engine.new(input: [scripted(:yes_fine, name: 'sloppy')])
    result = engine.check_input('hello')
    refute result.certain?
  ...
  def test_rejects_an_unknown_on_error
    assert_raises(ArgumentError) { NemoGuardrails::Engine.new(on_error: :shrug) }
  end

  # --- reporting ---

  def test_offline_is_true_only_when_every_rail_is_offline
    offline = NemoGuardrails::Engine.new(input: [scripted(R.passed(rail: 'a'), offline: true)])
    assert offline.offline?
    mixed = NemoGuardrails::Engine.new(
  ...
  def test_an_empty_engine_is_not_offline
    refute NemoGuardrails::Engine.new.offline?
    assert NemoGuardrails::Engine.new.empty?
  end
  ...
  def test_describe_names_the_rails_per_side
    ...
    assert_equal 'no rails', NemoGuardrails::Engine.new.describe
  end

  def test_to_h_lists_rails_and_settings
    engine = NemoGuardrails::Engine.new(input: [scripted(R.passed(rail: 'a'), name: 'patterns')])
    h = engine.to_h
