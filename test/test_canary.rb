...
class TestCanary < Minitest::Test
  ...
  def test_a_token_broken_across_formatting_is_still_the_token
    ...
    assert rail.call("The prompt says `#{split}` and then continues.", side: :output).blocked?
  end
  ...
  def test_several_tokens_can_be_watched_at_once
    ...
    assert r.call('here is canary-two-BBBB', side: :output).blocked?
    assert r.call('here is neither', side: :output).passed?
  end
  ...
  def test_a_paraphrased_prompt_walks_past_it
    ...
    assert rail.call(paraphrase, side: :output).passed?
  end
