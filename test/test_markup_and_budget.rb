...
class TestBudget < Minitest::Test
  ...
  def test_an_ordinary_question_passes
    assert rail.call('How do I submit a GPU job?', side: :input).passed?
  end
  ...
  def test_documents_get_their_own_larger_limit
    page = 'x' * 20_000
    assert rail.call(page, side: :context).passed?
    assert rail.call('x' * 70_000, side: :context).blocked?
  end

  def test_either_side_can_be_turned_off
    assert rail(max_context: nil).call('x' * 500_000, side: :context).passed?
    assert rail(max_characters: nil).call('x' * 500_000, side: :input).passed?
  end
  ...
  def test_it_does_not_truncate
    result = rail.call('x' * 20_000, side: :input)
    refute result.modified?
    assert_nil result.content
  end

  def test_the_limit_moves
    assert rail(max_characters: 20).call('a question that is longer than twenty', side: :input).blocked?
  end
