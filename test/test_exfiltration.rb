...
class TestExfiltration < Minitest::Test
  ...
  def test_a_link_host_is_not_an_image_host_unless_it_is_named
    ...
    assert clean(text, allow_images: []).modified?
    assert clean(text, allow_images: [HANDBOOK]).passed?
  end
  ...
  def test_percent_encoded_text_counts_as_a_payload
    ...
    assert clean("[x](https://#{HANDBOOK}/a?d=#{encoded})").modified?
  end
  ...
  def test_non_http_schemes_never_pass
    %w[
      ...
      assert clean("[click](#{url})").modified?, url
    end
  end

  def test_protocol_relative_urls_do_not_slip_through
    assert clean('[click](//collector.example.net/p)').modified?
  end
  ...
  def test_an_autolink_is_a_link
    assert clean('<https://collector.example.net/p>').modified?
  end
  ...
  def test_ordinary_handbook_prose_is_left_alone
    ...
    ANSWER
    assert clean(text).passed?, 'a plain answer was edited'
  end
  ...
  def test_the_default_allowlist_is_empty
    ...
    assert plain.call('[a](https://example.org/x)', side: :output).modified?
  end
